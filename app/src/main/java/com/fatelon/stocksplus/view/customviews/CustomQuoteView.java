...
public class CustomQuoteView extends LinearLayout {
    ...
    public void setQuote(OneQuoteDTO oneQuoteDTO) {
        ...
        try {
            if (oneQuoteDTO.getChageNum().charAt(0) == '+') customQuoteChageNum.setTextColor(ContextCompat.getColor(getContext(), R.color.green));
            else customQuoteChageNum.setTextColor(ContextCompat.getColor(getContext(), R.color.red));
            if (oneQuoteDTO.getChangePercent().charAt(0) == '+') customQuoteChangePercent.setTextColor(ContextCompat.getColor(getContext(), R.color.green));
            else customQuoteChangePercent.setTextColor(ContextCompat.getColor(getContext(), R.color.red));
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }
